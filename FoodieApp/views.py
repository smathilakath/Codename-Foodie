...
from FoodieApp.models import UserInfo, Menu, Restaurant, Address, Order
...
def home(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            print "Here I am, I came to see you"
    ...
    # if a GET (or any other method) we'll create a blank form
        ...
        print "else I came to see you"
        print request.method
        form = LoginForm()
    print "Hey, I came to see you"
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and len(form.cleaned_data) >= 6:
            print "Here I am"
            print "form valid"
            u = User.objects.create_user(username=form.cleaned_data["username"], email=form.cleaned_data[
...
def restaurantlist_view(request, district_name, food_name):
    ...
    ident_restaurant2 = Restaurant.objects.filter(
        menu__name=food_name, district=district_name)

    ident_restaurant = Restaurant.objects.filter(
        menu__name=food_name, district=district_name).values_list('id', flat=True)
    ident_menu = Menu.objects.filter(
        restaurant_id__in=ident_restaurant, name=food_name)
